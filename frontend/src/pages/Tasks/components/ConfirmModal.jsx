...
const ConfirmModal = ({ deleteModal, setDeleteModal, taskToDelete, fetchTasks }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL}/api/tasks/${taskToDelete._id}`, {
    ...
    <div>
      ...
      >
        <div className='flex justify-between items-center px-5 py-1.5 sm:py-3 font-bold border-b border-white/10'>
          <h1 className='w-full text-sm sm:text-xl '>Delete Task</h1>
        ...
        <div className='w-full h-auto my-1 sm:my-3 flex items-center justify-center px-5 py-3 text-xs sm:text-sm text-gray-500'>
          <p>This will permanently delete this task and all its data. This action cannot be undone.</p>
        </div>

...
const Navbar = ({ data ,sidebarOpen, setSidebarOpen}) => (
    <nav className="sticky top-0 z-10 w-full h-13 md:h-16 border border-white/10 text-white flex items-center px-2 md:px-5 justify-between md:gap-5">
        ...
        <div className="relative hidden md:flex flex-1 items-center ml-5">
            ...
            <input placeholder="Search goals, projects, tasks..." className="outline-none pl-10 pr-2 py-0.5 bg-blue-500/5 w-full max-w-md h-10 border border-white/30 rounded-xl" type="text" /> </div>
        ...
        <div className="flex flex-1 items-center justify-end gap-2  md:gap-10 my-3">
            <NavLink to="/activity" className="h-full min-w-10 flex justify-center items-center gap-1 px-3 py-1 bg-amber-400/20 rounded-full border-2 border-red-400/20 whitespace-nowrap" >
                ...
                <h3 className="text-amber-500 font-medium text-xs md:text-lg">{data?.currentStreaksCount}</h3>
